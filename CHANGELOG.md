...
- Direct-message send receipts now name the exact requested and resolved recipients and report unconfirmed enqueue without claiming delivery.
- Empty reader lists now surface a queued-or-unread signal instead of requiring callers to infer delivery state.
- Direct-message docs now explain that `wait` injects on idle while `steer` injects immediately and may interrupt active work.
